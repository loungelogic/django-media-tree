...
class SimpleFileNodeChangeList(ChangeList):
    ...
    def get_results(self, request):
        """ Temporarily decreases the `level` attribute of all search
            results in order to prevent indendation when displaying them. """

        super(SimpleFileNodeChangeList, self).get_results(request)
...
class FileNodeChangeList(MPTTChangeList):

    def is_filtered(self, request):
        return is_search_request(request) or self.params

    def __init__(self, request, *args, **kwargs):
        super(FileNodeChangeList, self).__init__(request, *args, **kwargs)
        # self.parent_folder is set in get_query_set()
        self.title = self.parent_folder.name if self.parent_folder \
            else FileNode.get_top_node().name
